...
public class MainViewTests {
	...
	public void shouldFillOutTheGridWithNewData() {
		...
		customerDataWasFilled(editor, "Marcin", "Grzejszczak", "Bel Air");
		...
		then(getCustomersInGrid().get(getCustomersInGrid().size() - 1))
			.extracting("firstName", "lastName", "address")
			.containsExactly("Marcin", "Grzejszczak", "Bel Air");
	...
	public void shouldFilterOutTheGridWithTheProvidedLastName() {
		...
		this.repository.save(new Customer("Josh", "Long", "G baie"));
		...
		then(getCustomersInGrid().get(getCustomersInGrid().size() - 1))
			.extracting("firstName", "lastName", "address")
			.containsExactly("Josh", "Long", "G baie");
	}
	...
	private void customerDataWasFilled(CustomerEditor editor, String firstName,
		...
		this.editor.address.setValue(address);
		editor.editCustomer(new Customer(firstName, lastName, address));
	}
	...
	static class Config {
		...
		public void initializeData() {
			this.repository.save(new Customer("Jack", "Bauer", "A"));
			this.repository.save(new Customer("Chloe", "O'Brian", "B"));
			this.repository.save(new Customer("Kim", "Bauer", "C"));
			this.repository.save(new Customer("David", "Palmer", "D"));
			this.repository.save(new Customer("Michelle", "Dessler", "E"));
		}
	}
}
